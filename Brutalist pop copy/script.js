...
reveal(
  "#ctaBtn",
  { scale: 0.8 },
  { scale: 1, duration: 0.75, ease: "elastic.out(1,0.6)" },
  0.5,
);
reveal(
  "#footerSymbols",
  { y: 16 },
  { y: 0, duration: 0.65, ease: "back.out(2)" },
  0.5,
);

// Cards: stagger by column + tilt on hover (no click)
...
document.fonts.ready.then(() => {
  ...
  new IntersectionObserver(
    ...
    { threshold: 0.2 },
  ).observe(document.querySelector("#manifestoQuote"));
  ...
  new IntersectionObserver(
    ...
    { threshold: 0.2 },
  ).observe(document.querySelector("#ctaTitle"));
});
